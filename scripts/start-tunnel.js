...
async function startCloudflareTunnel() {
    ...
    proc.on('error', (err) => {
        console.error('❌ [Cloudflare Spawn Error]:', err.message);
        process.exit(1);
    });
...
async function startNgrokTunnel() {
    ...
    for (let i = 0; i < 25; i++) {
        ...
        try {
            const res = await new Promise((resolve, reject) => {
                http.get('http://127.0.0.1:4040/api/tunnels', { timeout: 1000 }, (r) => {
                    let d = '';
                    r.on('data', c => d += c);
                    r.on('end', () => {
            ...
            if (res && res.tunnels && res.tunnels.length > 0) {
                const httpsTunnel = res.tunnels.find(t => t.public_url && t.public_url.startsWith('https://')) || res.tunnels[0];
                if (httpsTunnel && httpsTunnel.public_url) {
                    detectedUrl = httpsTunnel.public_url.replace(/\/+$/, '');
                    break;
                }
            }
        } catch (e) {}
    ...
    proc.on('close', (code) => {
        console.log(`[ngrok] Tunnel exited with code ${code}`);
        process.exit(code || 0);
    });
